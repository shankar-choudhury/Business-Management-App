package com.spring2024project.Scheduler.entityFunctionalities.creditcard;
...
public class CreditCardService implements BaseService<CreditCard> {
    ...
    public CreditCard getById(int id) {
        return ccr.findById(id).orElse(CreditCard.emptyCreditCard());
    }
    ...
    public CreditCard create(CreditCard entity) {
        CreditCard newCard = CreditCard.checkedFrom(entity, zt);
    ...
    public CreditCard delete(int id) {
        ...
        if (toDelete.getId() != 0) {
            CreditCard deleted = CreditCard.fromDeleted(toDelete);
